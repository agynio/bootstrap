...
dump_diagnostics() {
  ...
  local workload
  for workload in egress egress-gateway runners ziti-management; do
    kubectl --kubeconfig "$KUBECONFIG_PATH" -n "$PLATFORM_NAMESPACE" \
      logs --all-containers --prefix --tail=100 -l "app.kubernetes.io/name=${workload}" || true
    kubectl --kubeconfig "$KUBECONFIG_PATH" -n "$PLATFORM_NAMESPACE" \
      logs --all-containers --prefix --previous --tail=100 -l "app.kubernetes.io/name=${workload}" || true
  done
